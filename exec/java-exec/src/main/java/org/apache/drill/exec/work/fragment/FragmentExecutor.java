...
public class FragmentExecutor implements Runnable {
  ...
  public FragmentExecutor(final FragmentContext context, final PlanFragment fragment,
    ...
    this.cancelExecutor = new CancelExecutor(fragmentName, fragmentContext.getExecutor());
   ...
   */
  public void cancel() {
    cancelExecutor.submitCancellation();
  }
  ...
  private class CancelExecutor extends SerializedExecutor {

    public CancelExecutor(String name, Executor underlyingExecutor) {
      super("cancel:" + name, underlyingExecutor);
    }
