...
final class PageReader {
    ...
    try {
      ...
      if (columnChunkMetaData.getDictionaryPageOffset() > 0) {
        ...
        if(parentColumnReader.columnChunkMetaData.getCodec()==CompressionCodecName.UNCOMPRESSED) {
          dataReader.getPageAsBytesBuf(pageDataByteArray, pageHeader.compressed_page_size);
        }else{
        ...
        DictionaryPage page = new DictionaryPage(
            getBytesInput(pageDataByteArray),
  ...
  public boolean next() throws IOException {
    ...
    final ByteBuffer pageDataBuffer = pageDataByteArray.nioBuffer(0, pageDataByteArray.capacity());
  ...
  private void allocatePageData(int size) {
    Preconditions.checkArgument(pageDataByteArray == null);
    pageDataByteArray = parentColumnReader.parentReader.getOperatorContext().getAllocator().buffer(size);
  }
