...
import org.apache.drill.exec.rpc.PlatformChecker;
...
public class BootStrapContext implements Closeable{
  ...
  public BootStrapContext(DrillConfig config) {
    super();
    this.config = config;
    this.loop = PlatformChecker.createEventLoopGroup(config.getInt(ExecConstants.BIT_SERVER_RPC_THREADS), "BitServer-");
    this.loop2 = PlatformChecker.createEventLoopGroup(config.getInt(ExecConstants.BIT_SERVER_RPC_THREADS), "BitClient-");
