...
public class JsonReader extends BaseJsonProcessor {
  ...
  private void confirmLast() throws IOException{
    parser.nextToken();
    Preconditions.checkArgument(parser.isClosed(), "Drill attempted to unwrap a toplevel list "
  ...
  private ReadState writeToVector(ComplexWriter writer, JsonToken t) throws IOException {
    ...
    case START_ARRAY:
      if(skipOuterList){
        if(inOuterList){
          throw new IOException("The top level of your document must either be a single array of maps or a set of white space delimited maps.");
        }
    ...
    case END_ARRAY:
      ...
      if(inOuterList){
        ...
        throw new IOException(String.format("Failure while parsing JSON.  Ran across unexpected %s.", JsonToken.END_ARRAY));
      }
